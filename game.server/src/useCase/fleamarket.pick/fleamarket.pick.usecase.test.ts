...
import { CardType, CharacterStateType, GlobalFailCode, RoomStateType, CharacterType, RoleType } from '../../generated/common/enums';
...
jest.mock('../../services/state.change.service');
...
describe('플리마켓 카드 선택 시나리오', () => {
  ...
  const setupTestEnvironment = () => {
    ...
    (roomManger.getUserFromRoom as jest.Mock).mockImplementation((roomId: number, userId: string) => {
    ...
    // stateChangeService 실제 동작 모킹
    (stateChangeService as jest.Mock).mockImplementation(
      (user: User, state?: CharacterStateType, nextState?: CharacterStateType) => {
        if (!user.character || !user.character.stateInfo) return;
        if (state !== undefined) user.character.stateInfo.state = state;
        if (nextState !== undefined) user.character.stateInfo.nextState = nextState;
  ...
  it('시나리오 3: 모든 생존 플레이어가 카드를 선택하면 플리마켓이 종료되고 상태가 초기화된다', () => {
    ...
    testRoom.roomFleaMarketCards = [CardType.BBANG, CardType.SHIELD, CardType.VACCINE, CardType.BOMB];
