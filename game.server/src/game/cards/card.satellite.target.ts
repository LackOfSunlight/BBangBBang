...
import { AnimationType, CardType, CharacterStateType } from '@core/generated/common/enums';
...
import { checkAndEndGameIfNeeded } from '@game/services/game.end.service';
import takeDamageService from '@game/services/take.damage.service';
import { ICard, IPeriodicEffectCard } from '@common/types/card';
...
export class SatelliteTargetCard implements ICard, IPeriodicEffectCard {
	...
	public async processSatelliteTargetEffect(user: User, room: Room, allUsers: any[]) {
		...
		if (isEffectTriggered) {
			...
			const nextTarget = rearrangedUsers.find((u) => u && u.character && u.character.hp > 0);

			if (!nextTarget) {
				return;
			}

			if (!nextTarget.character.debuffs.includes(CardType.SATELLITE_TARGET)) {
				nextTarget.character.debuffs.push(CardType.SATELLITE_TARGET);
			}
		}
	}
}
