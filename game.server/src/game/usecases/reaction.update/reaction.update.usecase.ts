...
import { getCard } from '@core/network/dispatcher/apply.card.dispacher';
		...
		if (user != null && user.character && user.character.stateInfo) {
			switch (user.character.stateInfo.state) {
				case CharacterStateType.BBANG_TARGET: {
					...
					// const bbangCard = getCard(CardType.BBANG) as BBangCard;
					...
					const damage = weaponDamageEffect(BBangCard.BBangDamage, shooter.character);
