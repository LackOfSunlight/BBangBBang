...
describe("cardWinLotteryEffect", () => {
    ...
    describe("카드 획득 로직", () => {
        ...
        it("같은 타입의 카드가 이미 있으면 count 증가", async () => {
            ...
            const bbangCard = mockUserWithExistingCards.character.handCards.find(card => card.type === CardType.BBANG);
            expect(bbangCard?.count).toBe(3);
            
            // VACCINE은 새로 추가
            expect(mockUserWithExistingCards.character.handCards).toContainEqual({ type: CardType.VACCINE, count: 1 });
        ...
        it("덱에 카드가 3장 미만이면 있는 만큼만 획득", async () => {
            ...
            expect(mockUpdateCharacterFromRoom).toHaveBeenCalledWith(roomId, userId, mockUserPartialDeck.character);
        });
    });
