...
function makeCharacter(hasBbang: boolean): CharacterData {
  return {
    ...
    equips: [],
    debuffs: [],
    handCards: hasBbang ? [{ type: CardType.BBANG, count: 1 }] : [{ type: CardType.SHIELD, count: 1 }],
    bbangCount: 0,
    handCardsCount: 1,
  };
}
...
describe('cardDeathMatchEffect', () => {
  ...
  describe('성공 케이스', () => {
    it('현피 카드 사용 시 두 플레이어의 상태를 올바르게 설정하고 Redis 업데이트 호출', async () => {
      ...
      mockUpdateCharacterFromRoom.mockResolvedValue(undefined as unknown as void);
    ...
    it('현피 카드 사용 시 콘솔 로그 출력 확인', async () => {
      ...
      // Then: 성공 로그가 출력되었는지 확인
      expect(consoleSpy).toHaveBeenCalledWith(
        `[현피] ${user.nickname}이 ${target.nickname}에게 현피를 걸었습니다.`
      );
  ...
  describe('실패 케이스', () => {
    ...
    it('빵야 카드가 없으면 조기 종료 및 로그 출력', async () => {
      ...
      const target: UserData = { id: targetUserId, nickname: '사용자2', character: makeCharacter(true) };
      ...
      // Then: 빵야 카드 없음 로그가 출력되고 업데이트는 하지 않아야 함
      expect(consoleSpy).toHaveBeenCalledWith('빵야 카드가 없습니다');
      expect(mockUpdateCharacterFromRoom).not.toHaveBeenCalled();
      consoleSpy.mockRestore();
    });

    it('Redis 업데이트 실패 시 에러 로그 출력', async () => {
      // Given: 정상적인 현피 카드 사용이지만 Redis 업데이트가 실패하는 상황
      const roomId = 1;
      const userId = 'user1';
      const targetUserId = 'user2';
