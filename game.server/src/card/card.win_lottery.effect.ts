...
const cardWinLotteryEffect = async (roomId: number, userId: string) => {
    ...
    user.character!.handCardsCount = user.character!.handCards.reduce((total, card) => total + card.count, 0);
    
    // Redis에 업데이트된 캐릭터 정보 저장
    // S2CUserUpdateNotification으로 클라이언트에 전송됨
    try {
        await updateCharacterFromRoom(roomId, user.id, user.character);
        console.log(`[복권 당첨] ${user.nickname}이 카드 ${newCardTypes.length}장을 획득했습니다:`, 
