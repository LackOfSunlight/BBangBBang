...
describe("reactionUpdateHandler + reactionUpdateUseCase", () => {
  ...
  it("BBANG_TARGET이고 자동 실드가 없다면 체력 1 감소 및 상태 초기화", async () => {
    ...
    mockRoom.users.push({
      ...
      character: {
        ...
        equips: [],
        bbangCount: 0,
        stateInfo: { state: CharacterStateType.NONE_CHARACTER_STATE, stateTargetUserId: "0", nextStateAt: "0" },
      },
    });
  ...
  it("BIG_BBANG_TARGET이면 체력 1 감소하고 CheckBigBbangService을 호출", async () => {
    ...
    // mock service
    const checkBigBbangSpy = jest.spyOn(require("../../services/bigbbang.check.service"), "CheckBigBbangService")
  ...
  it("GUERRILLA_TARGET 이면 체력 1감소 및 CheckGuerrillaService호출", async () => {
    ...
    const checkGuerrillaSpy = jest.spyOn(require("../../services/guerrilla.check.service"), "CheckGuerrillaService")
      .mockResolvedValue(mockRoom);
  ...
  it("DEATH_MATCH_TURN_STATE이면 체력 1 감소 및 양측 플레이어 상태 초기화", async () => {
    ...
    expect(user.character.hp).toBe(2);
    expect(user.character.stateInfo.state).toBe(CharacterStateType.NONE_CHARACTER_STATE);
    expect(mockRoom.users[1].character.stateInfo.state).toBe(CharacterStateType.NONE_CHARACTER_STATE);
  });
