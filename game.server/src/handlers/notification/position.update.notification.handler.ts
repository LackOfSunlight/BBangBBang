    ...
    if (gamePacket.payload.oneofKind === "positionUpdateNotification") {
        ...
        for (const pos of positionData.characterPositions) {
            if (!pos.id || typeof pos.x !== 'number' || typeof pos.y !== 'number') {
                console.warn(`[PositionUpdateNotification] Invalid position data - id: ${pos.id}, x: ${pos.x}, y: ${pos.y}`);
                return;
            }
        }
    }
...
export const setPositionUpdateNotification = (characterPositions: CharacterPositionData[]): GamePacket => {
