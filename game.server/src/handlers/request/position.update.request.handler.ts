...
import positionUpdateNotificationHandler, { setPositionUpdateNotification } from "../notification/position.update.notification.handler.js";
...
const positionUpdateRequestHandler = (socket: GameSocket, gamePacket: GamePacket) => {
    ...
    if (!payload || !socket.userId || !socket.roomId) {
        console.warn(`[PositionUpdate] Invalid request - userId: ${socket.userId}, roomId: ${socket.roomId}`);
        return;
    }
