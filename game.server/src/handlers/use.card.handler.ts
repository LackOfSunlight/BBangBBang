...
const useCardHandler = async (socket: GameSocket, gamePacket: GamePacket) => {
	...
	/// 2. 유즈케이스 호출
	const { useCardResponse, useCardNotification, userUpdateNotification } = await useCardUseCase({
		userId,
		roomId,
		cardType,
		targetUserId,
	...
	const useCardResponsePacket = createUseCardResponsePacket(
		useCardResponse.success,
		useCardResponse.GlobalFailCode,
	);
	...
	if (useCardNotification && useCardNotification.targetUserId) {
		const useCardNotificationPacket = createUseCardNotificationPacket(
			useCardNotification.cardType,
			useCardNotification.userId,
			useCardNotification.targetUserId,
		);

		// 장착이 가능한가? equipCard : useCard
		if (useCardNotification.cardType >= 13 && useCardNotification.cardType <= 20)
			broadcastDataToRoom(
				room.users,
