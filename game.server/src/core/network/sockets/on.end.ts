...
import { checkAndEndGameIfNeeded } from '@game/services/game.end.service.js';
...
import { userUpdateNotificationPacketForm } from '@common/converters/packet.form.js';
	...
	try {
		...
		if (socket.roomId) {
			...
			if (room) {
				...
				// 기존 로직: 방에서 사용자 제거
				roomManger.removeUserFromRoom(Number(socket.roomId), socket.userId!);

				// 방에 사용자가 없으면 방 삭제
				if (room.users.length <= 0) {
					roomManger.deleteRoom(Number(socket.roomId));
				}
			}
		}
	} catch (error) {
